    ...
    <div className={cn("flex flex-col w-full max-w-[714px] gap-2", className)}>
      ...
      >
        ...
        <div className="relative w-full md:w-[319px] h-[258px] shrink-0 flex items-center justify-center">
          <div className="absolute inset-0 bg-gradient-to-br from-white/0 via-white/75 to-white/75 border border-white rounded-2xl opacity-50 pointer-events-none" />
          <img
            src={illustration}
        ...
        <div className="flex flex-col items-center md:items-start w-full gap-5">
          <div className="flex flex-col gap-2 w-full text-center md:text-left">
            <h3 className="font-['Inter'] font-bold text-[20px] leading-[24px] text-[#FF00E5]">
              {title}
            </h3>
            <p className="font-['Inter'] font-normal text-[14px] leading-[16px] text-[#364153]">
              {description}
            </p>
          </div>
          ...
          {partners.length > 0 && (
            <div className="flex flex-col gap-1 w-full items-center md:items-start">
              <span className="font-['Inter'] font-bold text-[12px] leading-[14px] text-[#1E2939]">
                Delivery Partners:
              </span>
          ...
          <button
            onClick={onCtaClick}
            className="mt-2 w-full md:w-auto px-6 py-2 bg-[#4D179A] rounded-[4px] hover:bg-[#3d127a] transition-colors shadow-md active:scale-95"
          >
            <span className="font-['Inter'] font-bold text-[14px] leading-[16px] text-white text-center block">
              {ctaText}
            </span>
          </button>
        </div>
      ...
      {servicesCovered.length > 0 && (
        <div className="flex flex-col items-center xl:items-start p-4 md:px-4 md:py-2 gap-2 self-stretch bg-white rounded-b-[12px] w-full">
          <h4 className="font-['Inter'] font-bold text-[16px] leading-[20px] text-[#1E2939]">
            Services Covered
          </h4>
          ...
          <div className="flex flex-row flex-wrap justify-center xl:justify-start items-start gap-[5px] w-full">
            {servicesCovered.map((service, idx) => (
              <div
                key={idx}
                className="flex flex-row justify-center items-center px-4 py-2 gap-[10px] bg-[#FAF5FF] rounded-[4px] h-[32px]"
              >
                <span className="font-['Inter'] font-medium text-[14px] leading-[16px] text-[#7F22FE] whitespace-nowrap">
                  {service}
                </span>
              </div>
            ))}
