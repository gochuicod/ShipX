...
import Container from "../components/library/Container"; // Ensure this path is correct

import { CalendarDays } from "lucide-react";

const ContactItem = ({ icon: Icon, text, href, itemClass, iconClass }) => (
  <li className={`flex gap-2 text-gray-600 ${itemClass}`}>
...
const FooterColumn = ({ title, children, className }) => (
  <div
    className={`bg-white rounded-[10px] p-4 md:p-[15px_20px] h-fit ${className}`}
  >
    <h3 className={themeGuide.footerHeading + " mb-4 md:mb-1"}>{title}</h3>
    {children}
  </div>
);
...
const Footer = memo(() => {
  ...
  return (
    <div className="lg:pt-62 md:pt-34 lg:mt-0 mt-34 relative">
      {/* CTA Section Wrapper */}
      {/* 1. Absolute wrapper to hold position */}
      <div className="absolute lg:top-[120px] md:-top-10 top-0 left-0 right-0 w-full z-10">
        {/* 2. Container to align width with footer */}
        <Container>
          {/* 3. The Card (w-full to fill container) */}
          <div
            className="
              relative
              w-full
          ...
          >
            {/* Glow blobs */}
            <div className="absolute w-[256px] h-64 bg-[#FF00E5]/20 blur-3xl rounded-full top-1 left-0 pointer-events-none" />
            ...
            >
              ...
              <img
                ...
                className="
                  ...
                  h-auto
                  absolute
                  lg:top-[-150px] md:top-0 top-[-26vw]
      ...
      <footer className="bg-[#F3F4F6]">
        <Container
          className="w-full md:pt-[10vw] pt-[30vw] md:pb-[2vw] pb-[4vw]"
